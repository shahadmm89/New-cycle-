...
chk "MARCH = decision and payout finalized"             "grep -q \"label: 'MARCH', sub: 'Decision and payout\" src/Video.tsx"
...
chk "all five month markers are anchored"               "test \$(grep -c \"beat: 'timeline\" src/config/anchors.ts) -eq 5"
...
chk "narrator is Dan"                                   "grep -q \"voiceId: 'fvVBPXuE7f1iX3dZLKFy'\" src/config/voiceover.ts"
